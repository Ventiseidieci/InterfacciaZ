...
class AsyncConsole:
    """Gestione log asincrona per non bloccare la GUI"""
...
class ConicStudioApp(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("CONIC STUDIO - Lite Edition")
        self.geometry("600x650") # Finestra più compatta
    ...
    def _setup_ui(self):
        ...
        f_params = ttk.Frame(lbl_p)
        f_params.pack(fill=tk.X)
        
        ttk.Label(f_params, text="Angolo (°):").grid(row=0, column=0, padx=5)
        self.spin_angle = ttk.Spinbox(f_params, from_=0, to=60, width=5)
        self.spin_angle.set(16)
        self.spin_angle.grid(row=0, column=1)
        
        ttk.Label(f_params, text="Fade Base (mm):").grid(row=0, column=2, padx=5)
        self.spin_fade = ttk.Spinbox(f_params, from_=0, to=50, width=5)
        self.spin_fade.set(3.0)
        self.spin_fade.grid(row=0, column=3)
        
        ttk.Label(f_params, text="Tipo:").grid(row=0, column=4, padx=5)
        self.combo_type = ttk.Combobox(f_params, values=["outward", "inward"], width=8, state="readonly")
        self.combo_type.set("outward")
        self.combo_type.grid(row=0, column=5)
    ...
    def run_transform_stl(self):
        ...
        self.status_var.set("Elaborazione STL in corso...")
        self.update_idletasks() # CLICK IMMEDIATO
        ...
        def task():
            try:
                ...
                print(f"\n--- AVVIO STL ---")
                print(f"Base Piana: {fade}mm | Angolo: {angle}°")
                # refinements=0 per velocità massima
                self.geo_engine.transform_mesh(path, out, angle, ctype, refinements=0, fade_height=fade)
                
                print(f"File salvato: {os.path.basename(out)}")
    ...
    def run_backtransform(self):
        ...
        self.status_var.set("Elaborazione Gcode in corso...")
        self.update_idletasks() # CLICK IMMEDIATO
        ...
        def task():
            try:
                ...
                print(f"\n--- AVVIO GCODE ---")
                # generate_preview=False -> TURBO MODE
                self.gcode_engine.backtransform_gcode(path, out, angle, ctype, fade_height=fade, generate_preview=False)
                
                print(f"File Finale Pronto: {os.path.basename(out)}")
