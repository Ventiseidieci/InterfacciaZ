...
class gcodeTransformer():
    ...
    def __init__(self, file_name = None, folder_name = 'gcodes/', cone_angle=16, cone_type='outward', first_layer_height=0.2, x_shift=110, y_shift = 90):
    ...
    def set_cone_angle(self, value):
        self._cone_angle = value

    def get_cone_type(self):
        return self._cone_type

    def set_cone_type(self, value):
        self._cone_type = value

    def get_first_layer_height(self):
        return self._first_layer_height

    def set_first_layer_height(self, value):
        self._first_layer_height = value

    def get_x_shift(self):
        return self._x_shift

    def set_x_shift(self, value):
        self._x_shift = value
    ...
    def compute_angle_radial(self, x_old, y_old, x_new, y_new, inward_cone):
        """
    ...
    def compute_U_values(self, angle_array):
        """
    ...
    def translate_data(self, data, cone_type, translate_x, translate_y, z_desired, e_parallel, e_perpendicular):
        """
    ...
    def backtransform_file(self, path, cone_type, maximal_length, angle_comp, x_shift, y_shift, cone_angle_deg, z_desired, e_parallel, e_perpendicular):
        """
        ...
        if angle_comp == 'radial':
            backtransform_data = self.backtransform_data_radial
        ...
        data_bt = self.translate_data(data_bt, cone_type, x_shift, y_shift, z_desired, e_parallel, e_perpendicular)
        data_bt_string = ''.join(data_bt)
    ...
    def start(self):
        starttime = time.time()
        self.backtransform_file(path=self.get_file_name(), cone_type=self.get_cone_type(), maximal_length=0.5, angle_comp='radial', x_shift=self.get_x_shift(), y_shift=self.get_y_shift(),
                           cone_angle_deg=self.get_cone_angle(), z_desired=self.get_first_layer_height(), e_parallel=0, e_perpendicular=0)
        endtime = time.time()
